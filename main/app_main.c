#include <stdio.h>
#include <stdint.h>
#include <stddef.h>
#include <string.h>
#include "esp_wifi.h"
#include "esp_system.h"
#include "nvs_flash.h"
#include "esp_event.h"
#include "esp_netif.h"
#include "esp_event_base.h"
#include "freertos/event_groups.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/semphr.h"
#include "freertos/queue.h"
#include "lwip/err.h"
#include "lwip/sys.h"
#include "lwip/sockets.h"
#include "lwip/dns.h"
#include "lwip/netdb.h"
#include "esp_tls.h"
#include "esp_ota_ops.h"
#include <sys/param.h>
#include "esp_log.h"
#include "mqtt_client.h"
#include "driver/ledc.h"
#include "driver/gpio.h"
#include "driver/rmt.h"
#include "iot_knob.h"
#include "iot_button.h"
#include "cJSON.h"
#include "sdkconfig.h"
#include "esp_spi_flash.h"
#include "esp_smartconfig.h"
#include "lightbulb.h"
#include "ws2812.h"
#include "led_indicator.h"
#include "button_gpio.h"

int s_retry_num = 0;
char smg = NULL;
char data_up[128] = {0};
char arr[128] = "";

#define DiscoveryTopic "homeassistant/light/kitchen/config" // HA自发现主题
#define CmdTopic "homeassistant/light/kitchen/set"          // 命令接收主题
#define DataTopic "homeassistant/light/kitchen/state"       // 数据上报主题

#define LEDC_TIMER LEDC_TIMER_0
#define LEDC_MODE LEDC_LOW_SPEED_MODE
#define LEDC_OUTPUT_IO (5) // Define the output GPIO
#define LEDC_CHANNEL LEDC_CHANNEL_0
#define LEDC_DUTY_RES LEDC_TIMER_8_BIT // Set duty resolution to 13 bits
#define LEDC_FREQUENCY (5000)          // Frequency in Hertz. Set frequency at 5 kHz
#define SW  GPIO_NUM_2

#define GPIO_KNOB_A GPIO_NUM_10
#define GPIO_KNOB_B GPIO_NUM_3
#define GPIO_KEY GPIO_NUM_2
#define WS2812_LED_NUM 16
#define WS2812_LED_GPIO GPIO_NUM_0
#define BUTTON_GPIO GPIO_NUM_11

static const char *TAG = "MQTT_LIGHT";

#define WIFI_CONNECTED_BIT BIT0 // 成功连接的位
#define WIFI_FAIL_BIT BIT1      // 尝试最大次数失败的位
#define SMART_CONFIG_BIT    BIT2
/* 宏定义WiFi更新标识码、WiFi名称和密码 */
#define MY_WIFI_UPDATE  4096                /* 对数值进行修改表示更新NVS的WiFi名称和密码*/
#define MY_WIFI_SSID    "WiFi-William"
#define MY_WIFI_PASSWD  "passwd-william"

esp_mqtt_client_handle_t client_g = NULL;
knob_handle_t s_knob;
button_handle_t gpio_btn;
/* 定义一个WiFi连接事件标志组句柄 */
static EventGroupHandle_t wifi_event_group_handler;

 wifi_config_t wifi_config;
 nvs_handle wificfg_nvs_handler;
 button_handle_t gpio_btn_rst;

int msg_id;
int pace = 5;
int brightness;
char state = NULL;
char up_msg[128];

char config[] = "{\n\"~\": \"homeassistant/light/kitchen\",\n\"name\": \"bedroomLight\",\n\"unique_id\": \"kitchen_light\",\n\"cmd_t\": \"~/set\",\n\"stat_t\": \"~/state\",\n\"schema\": \"json\",\n\"brightness\": true\n}";

static TimerHandle_t mqtt_timer;
void smartconfig_init_start(void);
/*初始化ws2812*/
void ws2812_init_cfg()
{
    // 1. 选择并配置芯片
    lightbulb_config_t config = {
        .type = DRIVER_WS2812,
        .driver_conf.ws2812.led_num = WS2812_LED_NUM,
        .driver_conf.ws2812.ctrl_io = WS2812_LED_GPIO,
        // 2. 配置驱动能力
        .capability.enable_fades = true,
        .capability.fades_ms = 800,
        .capability.enable_status_storage = false,
        .capability.mode_mask = COLOR_MODE,
        .capability.storage_cb = NULL,
        // 4. 极限参数
        .external_limit = NULL,
        // 5. γ参数
        .gamma_conf = NULL,
        // 6. 初始化参数
        .init_status.mode = WORK_COLOR,
        .init_status.on = true,
        .init_status.hue = 0,
        .init_status.saturation = 100,
        .init_status.value = 100,
    };
    esp_err_t err1 = lightbulb_init(&config);
    if (err1 != ESP_OK)
    {
        ESP_LOGE("LED_INIT", "There may be some errors in the configuration, please check the log.");
        return;
    }
}
/*MQTT回调回调函数*/
static void mqtt_timer_handler(TimerHandle_t xTimer)
{
    char data[128] = "";
    int duty = ledc_get_duty(LEDC_MODE, LEDC_CHANNEL);
    if (client_g != NULL)
    {
        if (duty == 0)
        {
            msg_id = esp_mqtt_client_publish(client_g, DataTopic, "{\"state\":\"OFF\"}", 0, 0, 0);
        }
        else
        {
            sprintf(data, "{\"state\":\"ON\",\"brightness\":%d}", duty);
            msg_id = esp_mqtt_client_publish(client_g, DataTopic, data, 0, 0, 0);
        }
    }
}

void button_init()
{
    // create gpio button
    button_config_t gpio_btn_cfg = {
        .type = BUTTON_TYPE_GPIO,
        .long_press_time = 1000,
        .short_press_time = 180,
        .gpio_button_config = {
            .gpio_num = GPIO_KEY,
            .active_level = 0,
        },
    };
    gpio_btn = iot_button_create(&gpio_btn_cfg);
    if(NULL == gpio_btn) {
        ESP_LOGE(TAG, "Button create failed");
    }
            // create gpio button
    button_config_t gpio_btn_cfg_rst = {
        .type = BUTTON_TYPE_GPIO,
        .long_press_time = 3000,
        .short_press_time = CONFIG_BUTTON_SHORT_PRESS_TIME_MS,
        .gpio_button_config = {
            .gpio_num = BUTTON_GPIO,
            .active_level = 0,
        },
    };
    gpio_btn_rst = iot_button_create(&gpio_btn_cfg_rst);
    if(NULL == gpio_btn_rst) {
        ESP_LOGE("TAG", "Button create failed");
    }
}

/* 按键重置wifi配置*/
static void button_single_click_cb_rst(void *arg,void *usr_data)
{
    bzero(&wifi_config, sizeof(wifi_config_t));
    nvs_flash_erase();
    smartconfig_init_start();
    esp_restart();
    ESP_LOGI("TAG", "BUTTON_SINGLE_CLICK");
}

static void button_single_click_cb(void *arg,void *usr_data)
{
    ESP_LOGI(TAG, "按钮按下并弹起");
    int duty = ledc_get_duty(LEDC_MODE, LEDC_CHANNEL);
    if(duty == 0)
    {
        ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 255);
        ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
    }else{
        ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 0);
        ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
    }
}

void knob_init()
{
    // create knob
    knob_config_t cfg = {
    .default_direction =0,
    .gpio_encoder_a = GPIO_KNOB_A,
    .gpio_encoder_b = GPIO_KNOB_B,
    };
    s_knob = iot_knob_create(&cfg);
    if(NULL == s_knob) {
        ESP_LOGE(TAG, "knob create failed");
    }
}

static void _knob_left_cb(void *arg, void *data)
{
    int pace_s = pace;
    if(brightness < -1)pace_s = 0;
    ESP_LOGI(TAG,"brightness-%d",pace_s);
    brightness -= pace_s;
    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, brightness);
    if(brightness >= 0 && brightness < 256)
    {
        ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
    }
}

static void _knob_right_cb(void *arg, void *data)
{
    int pace_s = pace;
    if(brightness >= 255)pace_s = 0;
    ESP_LOGI(TAG,"brightness+%d",pace_s);
    brightness += pace_s;
    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, brightness);
    if(brightness >= 0 && brightness < 256)
    {
        ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
    }
}

static void example_ledc_init(void)
{
    // Prepare and then apply the LEDC PWM timer configuration
    ledc_timer_config_t ledc_timer = {
        .speed_mode = LEDC_MODE,
        .timer_num = LEDC_TIMER,
        .duty_resolution = LEDC_DUTY_RES,
        .freq_hz = LEDC_FREQUENCY,
        .clk_cfg = LEDC_AUTO_CLK};
    ESP_ERROR_CHECK(ledc_timer_config(&ledc_timer));

    // Prepare and then apply the LEDC PWM channel configuration
    ledc_channel_config_t ledc_channel = {
        .speed_mode = LEDC_MODE,
        .channel = LEDC_CHANNEL,
        .timer_sel = LEDC_TIMER,
        .intr_type = LEDC_INTR_DISABLE,
        .gpio_num = LEDC_OUTPUT_IO,
        .duty = 0,
        .hpoint = 0};
    ESP_ERROR_CHECK(ledc_channel_config(&ledc_channel));
}

static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ESP_LOGD(TAG, "Event dispatched from event loop base=%s, event_id=%d", base, event_id);
    esp_mqtt_event_handle_t event = event_data;
    client_g = event->client;
    switch ((esp_mqtt_event_id_t)event_id)
    {
    case MQTT_EVENT_CONNECTED:
        ESP_LOGI(TAG, "MQTT 事件已连接");
        msg_id = esp_mqtt_client_subscribe(client_g, CmdTopic, 0);
        mqtt_timer = xTimerCreate("mqtt_timer", 10, pdTRUE, NULL, mqtt_timer_handler);
        xTimerStart(mqtt_timer, portMAX_DELAY);
        ESP_LOGI(TAG, "发送订阅成功, msg_id=%d", msg_id);
        break;
    case MQTT_EVENT_DISCONNECTED:
        ESP_LOGI(TAG, "MQTT已断开");
        xTimerStop(mqtt_timer, portMAX_DELAY);
        break;
    case MQTT_EVENT_SUBSCRIBED:
        ESP_LOGI(TAG, "MQTT已订阅, msg_id=%d", event->msg_id);
        msg_id = esp_mqtt_client_publish(client_g, DiscoveryTopic, config, 0, 0, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
        break;
    case MQTT_EVENT_UNSUBSCRIBED:
        ESP_LOGI(TAG, "MQTT取消订阅, msg_id=%d", event->msg_id);
        break;
    case MQTT_EVENT_PUBLISHED:
        ESP_LOGI(TAG, "MQTT已发布, msg_id=%d", event->msg_id);
        break;
    case MQTT_EVENT_DATA:
        ESP_LOGI(TAG, "MQTT数据以接收");
        sprintf(data_up, "%.*s", event->data_len, event->data);
        ESP_LOGI(TAG,"%s\n", data_up);
        int len = strlen(data_up);
        ESP_LOGI(TAG,"The data len is:%d\n", len);
        if (0 == strcmp(data_up, "{\"state\":\"OFF\"}"))
        {
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 0);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
        }
        if (0 == strcmp(data_up, "{\"state\":\"ON\"}"))
        {
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 255);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
            msg_id = esp_mqtt_client_publish(client_g, DataTopic, "{\"state\":\"ON\"}", 0, 0, 0);
        }
        if (len > 16)
        {
            cJSON *root = NULL;
            cJSON *brightness_s = NULL;
            cJSON *state_s = NULL;
            root = cJSON_Parse(data_up);
            if (root != NULL)
            {
                brightness_s = cJSON_GetObjectItem(root, "brightness");
                state_s = cJSON_GetObjectItem(root, "state");
                if (brightness_s != NULL && state_s != NULL)
                {
                    brightness = brightness_s->valueint;
                    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, brightness);
                    ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
                    ESP_LOGI(TAG,"The litgh is ON and the brightness is %d\n", brightness_s->valueint);
                    // sprintf(arr,"{\"state\":\"ON\",\"brightness\":%d}",brightness_s->valueint);
                    // msg_id = esp_mqtt_client_publish(client_g, DataTopic, arr, 0, 0, 0);
                }
                else
                {
                    ESP_LOGI("DEBUG", "The brightness_s and state_s is NULL");
                }
                cJSON_Delete(root);
            }
            else
            {
                ESP_LOGI("DEBUG", "root is NULL");
            }
        }
        break;
    case MQTT_EVENT_ERROR:
        ESP_LOGI(TAG, "MQTT错误");
        if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT)
        {
            ESP_LOGI(TAG, "esp-tls 报告的最后一个错误代码: 0x%x", event->error_handle->esp_tls_last_esp_err);
            ESP_LOGI(TAG, "最后 tls 堆栈错误号: 0x%x", event->error_handle->esp_tls_stack_err);
            ESP_LOGI(TAG, "最后捕获的errno : %d (%s)", event->error_handle->esp_transport_sock_errno,
                     strerror(event->error_handle->esp_transport_sock_errno));
        }
        else if (event->error_handle->error_type == MQTT_ERROR_TYPE_CONNECTION_REFUSED)
        {
            ESP_LOGI(TAG, "连接被拒绝: 0x%x", event->error_handle->connect_return_code);
        }
        else
        {
            ESP_LOGW(TAG, "未知错误类型: 0x%x", event->error_handle->error_type);
        }
        break;
    default:
        ESP_LOGI(TAG, "其他事件 ID:%d", event->event_id);
        break;
    }
}

static void mqtt_app_start(void)
{
    const esp_mqtt_client_config_t mqtt_cfg = {
        .host = "10.168.1.135",
        .port = 1883,
        .username = "homeassistant",
        .password = "homeassistant",
    };

    ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size());
    esp_mqtt_client_handle_t client = esp_mqtt_client_init(&mqtt_cfg);
    /* The last argument may be used to pass data to the event handler, in this example mqtt_event_handler */
    esp_mqtt_client_register_event(client, ESP_EVENT_ANY_ID, mqtt_event_handler, NULL);
    esp_mqtt_client_start(client);
}

/* 系统事件循环处理函数 */
static void event_handler(void* arg, esp_event_base_t event_base,int32_t event_id, void* event_data)
{
    static int retry_num = 0;           /* 记录wifi重连次数 */
    /* 系统事件为WiFi事件 */
    if (event_base == WIFI_EVENT) 
    {
        if(event_id == WIFI_EVENT_STA_START)    /* 事件id为STA开始 */
            esp_wifi_connect();
        else if (event_id == WIFI_EVENT_STA_DISCONNECTED) /* 事件id为失去STA连接 */ 
        {
            ws2812_set_rgb_channel(128,0,0);
            esp_wifi_connect();
            retry_num++;
            ESP_LOGI(TAG,"正在重连，重连次数 %d . \n",retry_num);
            if (retry_num > 5)  /* WiFi重连次数大于10 */
            {
                /* 将WiFi连接事件标志组的WiFi连接失败事件位置1 */
                xEventGroupSetBits(wifi_event_group_handler, WIFI_FAIL_BIT);
            }
            /* 清除WiFi连接成功标志位 */
            xEventGroupClearBits(wifi_event_group_handler, WIFI_CONNECTED_BIT);
        }
    } 
    /* 系统事件为ip地址事件，且事件id为成功获取ip地址 */
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) 
    {
        ws2812_set_rgb_channel(0,128,0);
        ip_event_got_ip_t* event = (ip_event_got_ip_t*) event_data; /* 获取IP地址信息*/
        ESP_LOGI(TAG,"got ip:%d.%d.%d.%d \n" , IP2STR(&event->ip_info.ip));  /* 打印ip地址*/
        retry_num = 0;                                              /* WiFi重连次数清零 */
        /* 将WiFi连接事件标志组的WiFi连接成功事件位置1 */
        xEventGroupSetBits(wifi_event_group_handler, WIFI_CONNECTED_BIT);
    }
    /* 系统事件为智能配网事件 */
    else if (event_base == SC_EVENT)
    {
        ws2812_set_rgb_channel(0,0,128);
        if(event_id == SC_EVENT_SCAN_DONE )             /* 开始扫描智能配网设备端 */
            ESP_LOGI(TAG,"扫描开始 |\n");
        else if(event_id == SC_EVENT_FOUND_CHANNEL)     /* 得到了智能配网通道 */
            ESP_LOGI(TAG,"得到配网通道 \n");
        else if(event_id == SC_EVENT_GOT_SSID_PSWD)     /* 得到了智能配网设备提供的ssid和password */
        {
            ESP_LOGI(TAG,"成功获得SSID和PASSWORD\n");
            /* 获取智能配网设备端提供的数据信息 */
            smartconfig_event_got_ssid_pswd_t *evt = (smartconfig_event_got_ssid_pswd_t *)event_data;
            /* 定义WiFi配置结构体和用了结收ssid和password的数组 */
           
            char ssid[32] = { 0 };
            char password[64] = { 0 };
            bzero(&wifi_config, sizeof(wifi_config_t)); /* 将结构体数据清零 */
            /* 将智能配网设备发送来的WiFi的ssid、password及MAC地址复制到wifi_config */
            memcpy(wifi_config.sta.ssid, evt->ssid, sizeof(wifi_config.sta.ssid));
            memcpy(wifi_config.sta.password, evt->password, sizeof(wifi_config.sta.password));
            wifi_config.sta.bssid_set = evt->bssid_set;
            if (wifi_config.sta.bssid_set == true) {
                memcpy(wifi_config.sta.bssid, evt->bssid, sizeof(wifi_config.sta.bssid));
            }
            
            /* 打印WiFi名称和密码 */
            memcpy(ssid, evt->ssid, sizeof(evt->ssid));
            memcpy(password, evt->password, sizeof(evt->password));
            ESP_LOGI(TAG,"SSID:%s \n", ssid);
            ESP_LOGI(TAG,"PASSWORD:%s \n", password);

            /* 将得到的WiFi名称和密码存入NVS*/
            
            ESP_ERROR_CHECK( nvs_open("WiFi_cfg", NVS_READWRITE, &wificfg_nvs_handler) );
            ESP_ERROR_CHECK( nvs_set_str(wificfg_nvs_handler,"wifi_ssid",ssid) );
            ESP_ERROR_CHECK( nvs_set_str(wificfg_nvs_handler,"wifi_passwd",password) );
            ESP_ERROR_CHECK( nvs_commit(wificfg_nvs_handler) ); /* 提交 */
            nvs_close(wificfg_nvs_handler);                     /* 关闭 */ 
            ESP_LOGI(TAG,"SSID和密码存入nvs .\n");

            /* 根据得到的WiFi名称和密码连接WiFi*/
            ESP_ERROR_CHECK( esp_wifi_disconnect() );
            ESP_ERROR_CHECK( esp_wifi_set_config(ESP_IF_WIFI_STA, &wifi_config) );
            ESP_ERROR_CHECK( esp_wifi_connect() );
        }
        else if (event_id == SC_EVENT_SEND_ACK_DONE)    /* 智能配网成功，已经给智能配网设备发送应答*/
        {
            xEventGroupSetBits(wifi_event_group_handler, SMART_CONFIG_BIT);
        }
    }
}
/*智能配网开始*/
void smartconfig_init_start(void)
{
    /* 设置智能配网类型为 ESPTOUCH */
    ESP_ERROR_CHECK( esp_smartconfig_set_type(SC_TYPE_ESPTOUCH) );
    /* 通过SMARTCONFIG_START_CONFIG_DEFAULT宏 来获取一个默认的smartconfig配置参数结构体变量*/
    smartconfig_start_config_t cfg = SMARTCONFIG_START_CONFIG_DEFAULT();
    /* 开始智能配网 */
    ESP_ERROR_CHECK( esp_smartconfig_start(&cfg) );
    ESP_LOGI(TAG,"配网开始 ....... \n");

    /* 使用事件标志组等待连接建立(WIFI_CONNECTED_BIT)或连接失败(WIFI_FAIL_BIT)事件 */
    EventBits_t uxBits;  /* 定义一个事件位变量来接收事件标志组等待函数的返回值 */
    /* 等待事件标志组，退出前清除设置的事件标志，任意置1就会返回*/
    uxBits = xEventGroupWaitBits(wifi_event_group_handler, WIFI_CONNECTED_BIT | SMART_CONFIG_BIT, 
                                    true, false, portMAX_DELAY); 
    if(uxBits & WIFI_CONNECTED_BIT) 
    {
        ESP_LOGI(TAG,"配网完成. \n");
        esp_smartconfig_stop(); /* 关闭智能配网 */
    }
    if(uxBits & SMART_CONFIG_BIT) 
    {
        ESP_LOGI(TAG,"配网完成. \n");
        esp_smartconfig_stop(); /* 关闭智能配网 */
    }
}

void wifi_init_sta(void)
{
        /* 初始化非易失性存储库 (NVS) */
    esp_err_t err = nvs_flash_init();
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        /* NVS分区被截断，需要删除,然后重新初始化NVS */
        ESP_ERROR_CHECK(nvs_flash_erase());
        err = nvs_flash_init();
    }
    ESP_ERROR_CHECK( err );

    /* 打开一个NVS命名空间 */
    ESP_ERROR_CHECK( nvs_open("WiFi_cfg", NVS_READWRITE, &wificfg_nvs_handler) );

    uint32_t wifi_update = 0;
    err = nvs_get_u32(wificfg_nvs_handler,"wifi_update",&wifi_update);
    if(MY_WIFI_UPDATE == wifi_update )
        ESP_LOGI(TAG,"wifi_cfg needn't to update. \n");
    else
    {
        ESP_LOGI(TAG,"wifi_cfg update now... \n");
        ESP_ERROR_CHECK( nvs_set_str(wificfg_nvs_handler,"wifi_ssid",MY_WIFI_SSID) );
        ESP_ERROR_CHECK( nvs_set_str(wificfg_nvs_handler,"wifi_passwd",MY_WIFI_PASSWD) ); 
        ESP_ERROR_CHECK( nvs_set_u32(wificfg_nvs_handler,"wifi_update",MY_WIFI_UPDATE) );
        ESP_LOGI(TAG,"wifi_cfg update ok. \n");
    }
    ESP_ERROR_CHECK( nvs_commit(wificfg_nvs_handler) ); /* 提交 */
    nvs_close(wificfg_nvs_handler);                     /* 关闭 */
 
    /* 创建一个事件标志组 */
    wifi_event_group_handler = xEventGroupCreate();

    /* 初始化底层TCP/IP堆栈。在应用程序启动时，应该调用此函数一次。*/
    ESP_ERROR_CHECK(esp_netif_init());

    /* 创建默认事件循环,*/
    ESP_ERROR_CHECK(esp_event_loop_create_default());

    /* 创建一个默认的WIFI-STA网络接口，如果初始化错误，此API将中止。*/
    esp_netif_create_default_wifi_sta();				

    /* 使用WIFI_INIT_CONFIG_DEFAULT() 来获取一个默认的wifi配置参数结构体变量*/
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    /* 根据cfg参数初始化wifi连接所需要的资源 */
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));

	/* 将事件处理程序注册到系统默认事件循环，分别是WiFi事件、IP地址事件及smartconfig事件 */
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &event_handler, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &event_handler, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(SC_EVENT, ESP_EVENT_ANY_ID, &event_handler, NULL));

    char wifi_ssid[32] = { 0 };     /* 定义一个数组用来存储ssid*/
    char wifi_passwd[64] = { 0 };   /* 定义一个数组用来存储passwd */
    size_t len;
    /* 打开一个NVS命名空间 */
    ESP_ERROR_CHECK( nvs_open("WiFi_cfg", NVS_READWRITE, &wificfg_nvs_handler) );
    len = sizeof(wifi_ssid);    /* 从NVS中获取ssid */
    ESP_ERROR_CHECK( nvs_get_str(wificfg_nvs_handler,"wifi_ssid",wifi_ssid,&len) );
    len = sizeof(wifi_passwd);      /* 从NVS中获取ssid */
    ESP_ERROR_CHECK( nvs_get_str(wificfg_nvs_handler,"wifi_passwd",wifi_passwd,&len) );
    ESP_ERROR_CHECK( nvs_commit(wificfg_nvs_handler) ); /* 提交 */
    nvs_close(wificfg_nvs_handler);                     /* 关闭 */
    /* 设置WiFi连接的ssid和password参数 */
    wifi_config_t wifi_config;
    bzero(&wifi_config, sizeof(wifi_config_t)); /* 将结构体数据清零 */
    memcpy(wifi_config.sta.ssid, wifi_ssid, sizeof(wifi_config.sta.ssid));
    memcpy(wifi_config.sta.password, wifi_passwd, sizeof(wifi_config.sta.password));

    /* 设置WiFi的工作模式为 STA */
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA) );
    /* 设置WiFi连接的参数，主要是ssid和password */
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_STA, &wifi_config));
    /* 启动WiFi连接 */
    ESP_ERROR_CHECK(esp_wifi_start());

    /* 使用事件标志组等待连接建立(WIFI_CONNECTED_BIT)或连接失败(WIFI_FAIL_BIT)事件 */
    EventBits_t bits;  /* 定义一个事件位变量来接收事件标志组等待函数的返回值 */ 
    bits = xEventGroupWaitBits( wifi_event_group_handler,	        /* 需要等待的事件标志组的句柄 */
                                WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,	/* 需要等待的事件位 */
                                pdFALSE,    /* 为pdFALSE时，在退出此函数之前所设置的这些事件位不变，为pdFALSE则清零*/ 
                                pdFALSE,    /* 为pdFALSE时，设置的这些事件位任意一个置1就会返回，为pdFALSE则需全为1才返回 */
                                portMAX_DELAY);	                    /* 设置为最长阻塞等待时间，单位为时钟节拍 */

    /* 根据事件标志组等待函数的返回值获取WiFi连接状态 */
    if (bits & WIFI_CONNECTED_BIT)  /* WiFi连接成功事件 */
	{
        ESP_LOGI(TAG,"成功连接到 %s  \n",wifi_ssid);
        vEventGroupDelete(wifi_event_group_handler);    /* 删除WiFi连接事件标志组，WiFi连接成功后不再需要 */
    } 
	else if (bits & WIFI_FAIL_BIT) /* WiFi连接失败事件 */
	{
        ESP_LOGI(TAG,"连接失败的wifi %s. \n",wifi_ssid);
        smartconfig_init_start();   /* 开启智能配网 */
    } 
	else 
    {
        ESP_LOGI(TAG,"无事件，开始配网 \n");  /* 没有等待到事件 */
        smartconfig_init_start();   /* 开启智能配网 */
    }
}


void app_main(void)
{
    knob_init();
    button_init();
    ws2812_init_cfg();
    example_ledc_init();
    wifi_init_sta();
    mqtt_app_start();
    iot_knob_register_cb(s_knob, KNOB_LEFT, _knob_left_cb, NULL);
    iot_knob_register_cb(s_knob, KNOB_RIGHT, _knob_right_cb, NULL);
    iot_button_register_cb(gpio_btn, BUTTON_PRESS_DOWN, button_single_click_cb,NULL);
    iot_button_register_cb(gpio_btn_rst, BUTTON_LONG_PRESS_START, button_single_click_cb_rst,NULL);
}

